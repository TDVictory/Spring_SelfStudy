...
## 第四章 面向切面的Spring
每家每户都会有一个电表，记录着我们每天的耗电情况。但是监控电量的消耗却不是大多数家庭关注的重点，而这却又是一个很重要的功能。

而在软件开发中，散布于应用中多处的功能被称为横切关注点（cross-cutting concern）。通常来讲，这些横切关注点从概念上是与应用的业务逻辑相分离的（但是往往胡直接嵌入到应用的业务逻辑之中）。把这些横切关注点与业务逻辑相分离正是面向切面编程（AOP）所要解决的问题。
...
#### 4.1.1 定义AOP术语
与大多数技术一样，AOP已经形成了自己的术语。常用术语有通知（advice），切点（pointcut）和连接点（join point）。接下来我们继续以电力公司运作为例来描述AOP运作。

**通知（Advice）**
电力公司的抄表员需要登记用电量并向电力公司报告。类似的切面也有其工作目标——它必须完成的工作。在AOP术语中，切面的工作被称为通知。

通知定义了切面是什么，以及何时使用。出了描述切面要完成的工作，通知还解决了合适执行这个工作的问题。它应该应用在某个方法被调用前还是调用后？还是在抛出异常时调用？

Spring切面可以应用5种类型的通知：
- 前置通知（Before）：在目标方法被调用前调用此通知功能。
- 后置通知（After）：目标方法完成之后调用通知，此时不会关心方法的输出是什么。
- 返回通知（After-returning）：目标方法成功执行之后调用通知，
- 异常通知（After-throwing）：目标方法抛出异常后调用通知。
- 环绕通知（Around）：目标方法调用前后均执行的自定义行为。

**连接点（Join point）**
电力公司为多个住户提供服务，甚至可能是一个城市。每家都有一个电表，这些电表上的数字都需要读取，因此每家都是抄表员的潜在目标。

同样的，我们的应用可能也有数以千计的时机应用通知。这些时机被称为连接点，连接点实在应用执行过程中能插入切面的一个点。这个点可以是调用方法时、抛出异常时、甚至修改一个字段时。切面代码可以利用这些点插入到应用的正常流程中，并添加新的行为。

**切点（Pointcut）**
一位抄表员不可能访问所有的电力公司的客户。每个抄表员都会被分配到指定一块区域的住户。类似的，一个切面并不需要通知应用所有的连接点，切点有助于缩小切面所通知的连接点的范围。

如果说通知定义了切面的“什么”和“何时”的话，切点就定义了“何处”。切点的定义会匹配通知所要织入的一个或多个连接点。

**切面（Aspect）**
抄表员每天都需要知道他今天要在什么时候，什么地方，办什么事。

切面是切点和通知的结合。切面是什么，将要在何时何处完成其功能。
